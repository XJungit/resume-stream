...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var ResumeCutNodeView = react.memo(function ResumeCutNodeView(props) {
      var node = props && props.node
      var data = (node && node.data) || {}
      var right =
        'TRANSPORT 容错' +
        (typeof data.count === 'number' ? ' · 累计 ×' + data.count : '')
      var detail =
    ...
    var resumeCutDefinition = {
      ...
      start: function (_context, match) {
        return {
          ...
          count: typeof match.event.data.count === 'number' ? match.event.data.count : null,
          seq: match.event.seq
        }
      },
    ...
    function apply(ctx) {
      ...
      if (conversationEvents !== undefined) {
        ...
        // Older DSH without the conversation Definition registry: degrade to a
        // badge-less plugin (auto-resume itself lives entirely in the host).
        console.warn('[resume-stream] conversationEvents 服务不可用，徽章已降级为不显示（续传功能不受影响）')
      }
