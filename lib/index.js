...
// content but stalls an in-flight task until the user nudges it. For GENUINE
// mid-stream breaks ("Connection error.", ECONNRESET, premature close, …) we
// queue a synthetic followup on the live Agent (`agents.get(id)` +
// `agent.followup(...)`, same mechanism dsh-goal-round-driver uses), so the
// driver wakes and opens the next turn by itself. Clean Zen-style EOFs
// ("Stream ended without finish_reason" — body fully delivered, only the
// finish marker missing) are NOT continued: the answer is already complete and
// asking the model to "continue" only produces filler. Guards: max consecutive
// auto-continues per session (reset by any clean completion) plus a short
// dedupe window against concurrent double-fired streams.
//
...
const CLEAN_EOF_RE = /without finish_reason/i
...
async function readStore(fs, path) {
...
function loadStore(ctx) {
  ...
  ;(async () => {
    try {
      let text = await readStore(fs, STORE_PATH)
      if (typeof text !== 'string' || text.length === 0) {
        // One-shot migration from the legacy root-level file, if present.
        text = await readStore(fs, LEGACY_STORE_PATH).catch(() => undefined)
      }
      if (typeof text === 'string' && text.length > 0) {
 ...
 * Only for genuine truncation — clean EOFs mean the answer was already whole.
 */
function tryAutoContinue(ctx, sessionId, failureMessage) {
  if (sessionId === undefined || sessionId === null) return
  if (CLEAN_EOF_RE.test(String(failureMessage ?? ''))) return
  const now = Date.now()
  ...
  const message = {
    ...
    content: [
      {
        type: 'text',
        text: '[resume-stream] 上一条回复在传输中途被切断（内容已尽量保留）。请从中断处继续完成剩余工作；若任务实际已经完成，请直接给出简短的收尾确认，不要重复已有内容。',
      },
    ],
  ...
  try {
    ...
    log(`auto-continue queued | session=${sessionId} chain=${chain + 1}/${MAX_AUTO_CONTINUES}`)
