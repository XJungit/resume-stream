...
export function apply(ctx) {
  ...
  // NOTE: resume-stream ships as a bundle package (cordis.patch.yml). Bundle
  // packages do NOT receive the `harness` Host Builtin nor a Client `host`
  // RPC handle — that Package-private Client↔Host RPC (harness.handle /
  // host.call) is exclusive to dynamic cordis plugins (cordis_define). Bundle
  // clients also cannot observe custom host→client data. So the live counter
  // is persisted to disk (resume-stream-count.json) and the log only; the
  // client half shows a static "auto-resume enabled" badge instead of a live
  // count. If a live count/tail notice is required, rebuild as a dynamic
  // cordis plugin.
