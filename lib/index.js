...
// to the live session. The client half subscribes to `session/event` and shows
// a per-turn tail badge ONLY for turns that were resumed — the bundle-package
// host→client channel (bundle packages have no host.call; a custom event type
// would brick session reload, so we reuse the inert known type).
//
...
function notifyCut(ctx, provider, code, message, session, turn) {
  ...
  // (no ctx.on consumer anywhere — host does not act on it, the client does not
  // render it), so it carries our signal without UI conflict or side effects.
  // The client filters on data.name === 'resume-stream-cut' and reads data.turn.
...
export function apply(ctx) {
  ...
  // `session/event` firehose. On every resume we append a KNOWN, inert
  // `hook/invoked` event (name:'resume-stream-cut'); the client half subscribes
  // and shows a per-turn tail badge ONLY for resumed turns. (A custom event type
  // would brick session reload, so we reuse the inert known type.)

  ctx.on('llm/stream', (options, next) => {
    log(`llm/stream hook fired | provider=${options?.provider ?? '?'} model=${options?.model ?? '?'} sessionId=${options?.sessionId ?? '?'}`)
    const stream = next()
    const sessions = ctx.get('sessions')
    const session = sessions !== undefined && options?.sessionId !== undefined
      ? sessions.get(options.sessionId)
      : undefined
    log(`  sessions=${sessions !== undefined} sessionFound=${session !== undefined}`)
